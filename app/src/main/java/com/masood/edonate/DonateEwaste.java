...
public class DonateEwaste extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_donate_ewaste);

        Toolbar toolbar = (Toolbar) findViewById(R.id.toolbar);
        toolbar.setTitle("Donate Ewaste");
        ...
        addressField = (EditText) findViewById(R.id.ewastePickup);
        quantityField = (EditText)findViewById(R.id.ewasteQuantity);
        username = (TextView) findViewById(R.id.userName);
        mDatabase = FirebaseDatabase.getInstance().getReference().child("Donors").child("Ewaste");
        mAuth = FirebaseAuth.getInstance();
        mCurrentUser =mAuth.getCurrentUser();
        mDatabaseUser = FirebaseDatabase.getInstance().getReference().child("DonorUser").child(mCurrentUser.getUid());
       }

    public void donateEwasteButtonClicked(View view){
        ...
        if(!TextUtils.isEmpty(address) && !TextUtils.isEmpty(quantity)) {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT) {
                ...
                final DatabaseReference Post = FirebaseDatabase.getInstance().getReference().child("Donors").child("Ewaste").child(userid).push();
...
//                    Post.child("username").setValue(dataSnapshot.child("username").getValue());
                        ...
                        Post.child("time").setValue(currentTime).addOnCompleteListener(new OnCompleteListener<Void>() {
                            ...
                            public void onComplete(@NonNull Task<Void> task) {
                                if (task.isSuccessful()) {
                                    Toast.makeText(DonateEwaste.this, "Your Request for E-Waste Donation is Successful", Toast.LENGTH_LONG).show();
                                    Intent intent = new Intent(DonateEwaste.this, Home.class);
                        ...
                        final DatabaseReference Posts = FirebaseDatabase.getInstance().getReference().child("Ngo").child("Ewaste").push();
                        mDatabaseUser.addValueEventListener(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
